...
public class WALEntryStream implements Iterator<Entry>, AutoCloseable, Iterable<Entry> {
  ...
  protected void dequeueCurrentLog() throws IOException {
    if (LOG.isTraceEnabled()) {
      ...
      LOG.trace(
        "Reached the end of log " + currentPath + " and the length of the file is " + filesize);
    }
    closeReader();
    logQueue.remove();
    currentPosition = 0;
  ...
  private void readNextEntryAndSetPosition() throws IOException {
    ...
    if (filter != null) {
      ...
      while (filterEntry(readEntry) == null && (readEntry = readNextEntry()) != null)
        ;
    }
  ...
  protected Entry readNextEntry() throws IOException {
    ...
    if (readEntry != null) {
      LOG.trace("Read wal entry: " + readEntry);
    } else {
      LOG.trace("No next entry from reader");
    }
    return readEntry;
  }
  ...
  private boolean openNextLog() throws IOException {
    ...
    if (nextPath != null) {
      if (!fs.exists(nextPath)) {
        ...
        Path archivedLogLocation = new Path(oldLogDir, nextPath.getName());
        if (fs.exists(archivedLogLocation)) {
          nextPath = archivedLogLocation;
          LOG.info("Log " + nextPath + " was moved to " + archivedLogLocation);
        } else {
          LOG.error("Couldn't locate log: " + nextPath);
          return false;
        }
      }
      openReader(nextPath);
