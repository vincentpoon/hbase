...
public class TestWALEntryStream {
  ...
  @Mock
  MetricsSource mockMetrics;

  @Test
  public void testMetricsStream() throws Exception {
    appendToLog("1");
    long position = 0;
    try (WALEntryStream entryStream =
        new MetricsWALEntryStream(walQueue, fs, conf, 0, null, mockMetrics)) {
      entryStream.next();
      verify(mockMetrics).incrLogEditsRead();
      log.rollWriter();
      appendToLog("2");
      entryStream.next();
      verify(mockMetrics).decrSizeOfLogQueue();
      assertFalse(entryStream.hasNext());
      position = entryStream.getPosition();
    }
    WALEntryFilter filterAll = new WALEntryFilter() {
      @Override
    ...
    appendToLog("3");
    try (WALEntryStream entryStream =
        new MetricsWALEntryStream(walQueue, fs, conf, position, filterAll, mockMetrics)) {
      assertFalse(entryStream.hasNext()); // no entries because they're all filtered
      verify(mockMetrics).incrLogEditsFiltered();
    }
  }
  ...
  public void testReplicationWALEntryBatcher() throws Exception {
    ...
    // start up a batcher
    ReplicationWALEntryBatcher batcher = new ReplicationWALEntryBatcher(getQueueInfo(), walQueue, 0,
